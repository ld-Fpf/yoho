...
<div class="right">
	...
	<tr>
		...
		<td><input type="text" name="name" value="<?php echo $data['name'];?>" class="inputall" /></td>
	</tr>
	<tr>
		...
		<td>
			<select name="category_id" class='inputall'>
				...
				<?php foreach ($cate as $k=>$v){?>
					<option value="<?php echo $v['category_id'];?>"     <?php if($data['category_id']==$v['category_id']){ ?>selected<?php } ?>><?php echo $v['_name'];?></option>
				<?php }?>
			</select>
...
<!-- 	<tr>
	...
	<tr>
		...
		<td>
			...
			<?php }else{ ?>
				<p><img src="http://localhost/yoho/<?php echo $data['logo'];?>" style="border:1px solid #ddd" /></p>
			<?php } ?>
			<input type="hidden" name="logoh" value="<?php echo $data['logo'];?>" />
			<p><input type="file" name="logo" /></p>
		</td>
	</tr>
	<tr>
		<td class="bg_f8" align="right">品牌排序：</td>
		<td><input type="text" name="sort" value="<?php echo $data['sort'];?>" class="inputall input50" /></td>
	</tr>
	<tr>
		<td class="bg_f8" align="right">热门品牌：</td>
		<td><input type="radio" name="is_hot" value="0"     <?php if($data['is_hot']==0){ ?>checked="checked"<?php } ?> />&nbsp;否&nbsp;&nbsp;<input type="radio" name="is_hot" value="1"     <?php if($data['is_hot']==1){ ?>checked="checked"<?php } ?> />&nbsp;是</td>
	</tr>
	<tr>
		<td class="bg_f8">&nbsp;</td>
		<input type="hidden" name="brand_id" value="<?php echo $hd['get']['bid'];?>" />
